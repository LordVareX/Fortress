...
APickupScrap::APickupScrap()
{
	...
	CollisionVolume->OnComponentBeginOverlap.AddDynamic(this, &APickupScrap::OnOverlapBegin);
...
void APickupScrap::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
// Called every frame
void APickupScrap::Tick(float DeltaTime)
{
...
void APickupScrap::OnConstruction(const FTransform& Transform)
{
...
void APickupScrap::OnOverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepHitResult)
{
...
void APickupScrap::CheckForExistingOverlaps()
{
...
void APickupScrap::AttemptPickUpWeapon_Implementation(APawn* Pawn)
{
...
void APickupScrap::StartCoolDown()
{
	if (UWorld* World = GetWorld())
	{
		World->GetTimerManager().SetTimer(CoolDownTimerHandle, this, &APickupScrap::OnCoolDownTimerComplete, CoolDownTime);
	}
}

void APickupScrap::ResetCoolDown()
{
	UWorld* World = GetWorld();
	...
	if (GetLocalRole() == ROLE_Authority)
	{
		...
		if (World)
		{
			World->GetTimerManager().SetTimer(CheckOverlapsDelayTimerHandle, this, &APickupScrap::CheckForExistingOverlaps, CheckExistingOverlapDelay);
		}
	}
...
void APickupScrap::OnCoolDownTimerComplete()
{
	ResetCoolDown();
}

void APickupScrap::SetWeaponPickupVisibility(bool bShouldBeVisible)
{
	WeaponMesh->SetVisibility(bShouldBeVisible, false);
}

void APickupScrap::PlayPickupEffects_Implementation()
{
...
void APickupScrap::PlayRespawnEffects_Implementation()
{
	if (WeaponDefinition != nullptr)
	{
...
void APickupScrap::OnRep_WeaponAvailability()
{
...
void APickupScrap::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(APickupScrap, bIsWeaponAvailable);
}

int32 APickupScrap::GetDefaultStatFromItemDef(const TSubclassOf<ULyraInventoryItemDefinition> WeaponItemClass, FGameplayTag StatTag)
{
