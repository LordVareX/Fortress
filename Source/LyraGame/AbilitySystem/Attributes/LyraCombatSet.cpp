...
void ULyraCombatSet::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION_NOTIFY(ULyraCombatSet, BaseEnergy, COND_OwnerOnly, REPNOTIFY_Always);
}
...
void ULyraCombatSet::OnRep_BaseEnergy(const FGameplayAttributeData& OldValue)
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(ULyraCombatSet, BaseEnergy, OldValue);
}
